...
def average_weights(w_list_of_state_dicts):
    """
    Performs FedAvg on a list of model state_dicts.
    Assumes all state_dicts are from models of the same architecture and are on CPU.
    Args:
        w_list_of_state_dicts (list): A list of PyTorch state_dicts.
    Returns:
        collections.OrderedDict: The averaged state_dict.
    """
    if not w_list_of_state_dicts:
        raise ValueError("Input list of state_dicts is empty for average_weights.")

    # Deepcopy the first state_dict to use as the structure for the average.
    # Ensures that operations are done on new memory.
    w_avg = copy.deepcopy(w_list_of_state_dicts[0])

    # Iterate over all keys in the state_dict
    for key in w_avg.keys():
        # Sum the weights for the current key from all other models
        for i in range(1, len(w_list_of_state_dicts)):
            w_avg[key] += w_list_of_state_dicts[i][key]
        # Divide by the total number of models to get the average
        w_avg[key] = torch.div(w_avg[key], float(len(w_list_of_state_dicts))) # Ensure float division
    return w_avg
